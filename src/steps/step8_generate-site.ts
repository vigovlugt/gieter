...
${cards}
  ...
  <div id="lightbox" onclick="handleLightboxClick(event)">
    ...
    <span class="lightbox-nav" id="lightbox-prev" onclick="lightboxNav(-1,event)">&#8249;</span>
    <img id="lightbox-img" src="" alt="">
    <span class="lightbox-nav" id="lightbox-next" onclick="lightboxNav(1,event)">&#8250;</span>
  ...
  <script>
    ...
    function selectPhoto(thumb, url) {
      ...
      card.querySelectorAll('.thumb').forEach(function(t) { t.classList.remove('thumb-active'); });
      thumb.classList.add('thumb-active');
    }

    function openLightbox(mainImg) {
      var raw = mainImg.dataset.photos;
      _lbPhotos = raw ? JSON.parse(raw) : [mainImg.src];
      _lbIndex = _lbPhotos.indexOf(mainImg.src);
    ...
    function _renderLightbox() {
      ...
      var showNav = _lbPhotos.length > 1;
    ...
    document.addEventListener('keydown', function(e) {
      ...
      if (e.key === 'ArrowLeft')  { _lbIndex = (_lbIndex - 1 + _lbPhotos.length) % _lbPhotos.length; _renderLightbox(); }
      if (e.key === 'ArrowRight') { _lbIndex = (_lbIndex + 1) % _lbPhotos.length; _renderLightbox(); }
    });
