...
function photoGallery(photos: string[], ref: string, lat?: number, lon?: number): string {
  ...
  return `
    <div class="gallery">
      ${photos.length > 0 ? `<img class="main-photo" src="${esc(photos[0])}" alt="main photo">` : ""}
...
function renderCard(listing: EnrichedListing, rank: number): string {
  ...
  const { algorithmic, ai, finalRating } = enrichment;
        ...
        <details class="scores-section">
          ...
          <div class="components">
            <div class="component-group">
              <h3>Algorithmic</h3>
              ${componentRow("Value for money", algorithmic.value)}
              ${componentRow("Social proof", algorithmic.socialProof)}
              ${componentRow("Practical amenities", algorithmic.practicalAmenities)}
            </div>
            <div class="component-group">
              <h3>AI assessment</h3>
...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .main-photo { width: 100%; height: 220px; object-fit: cover; border-radius: 8px; cursor: pointer; }
    ...
    .map { width: 100%; height: 180px; border-radius: 8px; overflow: hidden; margin-top: 2px; }
